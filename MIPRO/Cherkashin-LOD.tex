...
\section{Output LOD markup}
\label{sec:lod-out}

Resulting HTML document is exported in Word and Excel formats.  LOD markup is dome with comments of a special ``__RDF__'' user and contains list of \texttt{<RDFa attrinute>=<value>} pairs or a Turtle sentences.  Some pairs refer the commented Word document text as a relation literal value.  RDFa attribute name is written in form of RDFa of Turtle resource notation.
...
Excel table exports can be marked up also as ``Defined range''.  In this case the data exported can be used as input data for other tables computations..
...
% specifies the declination (case and number) for words being filled in
